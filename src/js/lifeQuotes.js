const div = document.getElementById("jsLifeQuotes");
...
const quotesArr = [
    ...
    "최선을 다하는 자는 후회하지 않는 자다",
    "끝날 때까지 끝난 게 아니다",
    "운동은 끝나고 먹는것까지가 운동이다",
    "운동할 때 힘이 든 것은 몸이 아니라 마음",
    "운동의 고통은 통증일 뿐, 힘든 것이 아니다",
];

const randomIndex = Math.floor(Math.random() * quotesArr.length);

const randomQuotes = quotesArr[randomIndex];

div.innerText = `"${randomQuotes}"`;
